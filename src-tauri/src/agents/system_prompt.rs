...
Allowed components:
  ...
  - `<Steps>` containing a markdown ordered list (`1. …`, `2. …`) of implementation steps. There is NO `<Step>` component — do NOT nest a `<Step>` tag; put a `1. …` markdown ordered list directly inside `<Steps>`. Each step should name what it reuses before what it adds. Optionally prefix a step's text with `done:` or `active:` to mark it complete or in-progress in the rendered stepper.
  ...
  - `<Wireframe label="..." surface="browser|app|mobile|panel|popover">` whose contents are low-fidelity mockup lines — one element per line, leading-space indentation = nesting. Containers: `row` (horizontal), `col` (vertical), `grid` (or `grid 3` for an N-column dense grid, 2–4), `section` (a labelled group), `box` (a card; its label becomes a header bar). Elements: `heading`/`text`/`field`/`input`/`pill`/`button`/`image`/`spacer`/`divider`. Each is optionally followed by a label (e.g. `button Sign in`). Use `spacer` inside a `row` to push items apart (logo left, actions right); use `grid`/`section`/`box` to convey real density and structure rather than a loose stack. `surface` picks the device/window chrome (defaults to `browser`). Compose real product content that fills the frame, not gray placeholder bars. Use it for a QUICK low-fidelity sketch. For anything that needs the actual look-and-feel (a real-looking component, fine interactions) prefer `<Preview>` instead.
  - `<Preview>` containing a self-contained React + Tailwind snippet that defines a component named `App` (plain JSX/TSX — do NOT use `import`; `React` is in scope; style with Tailwind utility classes). It renders as a LIVE, interactive UI in an isolated sandbox — a high-fidelity, real rendering of a proposed UI. Prefer `<Preview>` over `<Wireframe>` whenever you want the actual look-and-feel (an app shell, dashboard, a real component) rather than a rough sketch. The preview frame is LARGE (it grows with content up to ~960px tall), so design a generously-sized, real screen — the `App`'s root element should FILL the frame: give it a full-height root (e.g. `min-h-full` or a tall min-height like `min-h-[560px]`) and a background so it reads at real size, not a small card floating in empty space. Keep the snippet to a single screen; it loads its runtime from the network, so keep it dependency-free beyond React. A `<Preview>` may be the sole body of a `<Variant>`.
  - `<MultiPrototype>` containing 2–4 `<Variant label="..." recommended>` … `</Variant>` children — compares prototype options as tabs; mark the preferred one `recommended`. Each `<Variant>` MUST contain a single `<Wireframe>` (low-fi sketch) or `<Preview>` (live, real UI) and nothing else — every variant is a UI mockup, never plain prose or a description; a variant that isn't a mockup defeats the comparison. `<Variant>` is ONLY valid inside a `<MultiPrototype>`.
