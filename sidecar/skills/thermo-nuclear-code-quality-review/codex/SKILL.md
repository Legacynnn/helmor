...
description: Extremely strict maintainability and abstraction audit of a change or codebase. Use when the user wants a thorough, no-mercy code-quality review — spaghetti detection, the 1000-line rule, leaky abstractions, and "code-judo" restructurings. Helmor-vendored Codex variant.
---

# Thermo-Nuclear Code Quality Review (Codex)
...
Codex runs a single agent, so work the review as sequential focused passes over
the same target rather than parallel subagents. Make one pass per lens, write
down the findings for that lens, then move on — do not blur lenses together.
...
2. **Make one focused pass per lens, in order:**
...
3. **Synthesize.** Merge the per-pass findings, dedupe, and rank by severity
